#include "chunk.h"
// ---- offsets для каждой грани (UP, DOWN, FRONT, BACK, RIGHT, LEFT)
...
// --- Вспомогательная функция: вычислить UV для tileID на атласе
void GetAtlasUVs(int tileID, int atlasSize, int tileSize, Vector2 uv[4]) {
    ...
    // (левый-низ, правый-низ, правый-верх, левый-верх)
    uv[0] = { u0, v1 };
    uv[1] = { u1, v1 };
    uv[2] = { u1, v0 };
    uv[3] = { u0, v0 };
...
void Chunk::Draw() {
    ...
    rlBegin(RL_QUADS);
    rlColor3f(1.f, 1.f, 1.f); // Белый, чтобы не искажать текстуру
